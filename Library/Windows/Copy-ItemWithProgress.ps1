...
.SYNOPSIS 
Hydration Script Module - RoboCopy

.DESCRIPTION
Hydration Environment for MDTEx Powershell Common Modules
Robocopy Wrapper for PowerShell (Library Format)
...
#>
    ...
    $ScanOut  = [IO.Path]::GetTempFileName()
    $ScanErr  = [IO.Path]::GetTempFileName()
    ...
    write-Verbose "Robocopy Scan Out: [$ScanOut] Err: [$ScanErr]"
    $ScanRun = start-process robocopy -PassThru -WindowStyle Hidden -ArgumentList $ScanArgs -RedirectStandardOutput $ScanOut -RedirectStandardError $ScanErr
    ...
    if (($ScanRun.ExitCode -ge 10) -or (-not $LogData))
    {
        $LogData | write-Error
    ...
    remove-item $RoboLog, $ScanLog,$ScanOut,$ScanErr
